...
models_dir <- "/home/bhaddock/repos/bnAb_pnlme/single_mabs/5PL_pure_rlu/model_files"
model_files <- list.files(models_dir, pattern = "^5PL_m[0-9]+\\.mlxtran$")
model_names <- sub("^5PL_(m[0-9]+)\\.mlxtran$", "\\1", model_files)
model_names <- model_names[order(as.integer(sub("^m", "", model_names)))]

model_names <- paste0("m", 0:255)
...
run_one_model <- function(model_name, models_dir) {
  ...
  mlxtran_path <- file.path(models_dir, paste0("5PL_", model_name, ".mlxtran"))
