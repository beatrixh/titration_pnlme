...
BASE_MLXTRAN = Path("/mnt/c/Users/bhaddock/repos/titration_pnlme/single_mabs/5PL_edge_effects/5PL_edge_effects_m0.mlxtran")
MODEL_TRACKER_XLSX = Path("/mnt/c/Users/bhaddock/repos/titration_pnlme/single_mabs/5PL_edge_effects/model_tracker.xlsx")
OUTPUT_DIR = Path("/mnt/c/Users/bhaddock/repos/titration_pnlme/single_mabs/5PL_edge_effects/model_files")
...
def extract_distributions(text: str, params: list[str]) -> dict[str, str]:
    ...
    base file's own DEFINITION block, rather than assuming -- some
    parameters (e.g. m, s in this 5PL project) use `normal` instead of the
    usual `logNormal`, presumably to allow negative values.
    """
    ...
    for param in params:
        ...
        if not m:
            raise ValueError(f"could not find distribution for parameter '{param}' in base file")
        out[param] = m.group(1)
    return out
...
def build_individual_block(
    config: dict, categories: dict[str, list[str]], distributions: dict[str, str]
) -> str:
    used_covs = [
...
def generate_one(model_name: str, config: dict) -> None:
    ...
    output_mlxtran = OUTPUT_DIR / f"5PL_edge_effects_{model_name}.mlxtran"
    ...
    new_individual = build_individual_block(config, categories, distributions)
